...
namespace CarBook_1.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminServiceController : Controller
    {
        ...
        public string apiConnectedUrl = "https://localhost:7174/api/Services";
        ...
        {
            var responseMesage = await _client.DeleteAsync("https://localhost:7174/api/Services?id=" + id);
            if (responseMesage.IsSuccessStatusCode)
            {
        ...
        {
            var responseMesage = await _client.GetAsync($"https://localhost:7174/api/Services/{id}");
            if (responseMesage.IsSuccessStatusCode)
            {
                var jsonData = await responseMesage.Content.ReadAsStringAsync();
