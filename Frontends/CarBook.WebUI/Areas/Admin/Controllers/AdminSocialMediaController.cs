using CarBook_1.Dto.SocialMediaDtos;
...
namespace CarBook_1.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminSocialMediaController : Controller
    {
        ...
        public string apiConnectedUrl = "https://localhost:7174/api/SocialMedias";
        ...
        {
            var responseMesage = await _client.DeleteAsync("https://localhost:7174/api/SocialMedias?id=" + id);
            if (responseMesage.IsSuccessStatusCode)
            {
        ...
        {
            var responseMesage = await _client.GetAsync($"https://localhost:7174/api/SocialMedias/{id}");
            if (responseMesage.IsSuccessStatusCode)
            {
                var jsonData = await responseMesage.Content.ReadAsStringAsync();
